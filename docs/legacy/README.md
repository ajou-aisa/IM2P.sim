# LEGACY_BSV reference overview

> **LEGACY_BSV reference.** This document describes the retained BSV implementation,
> not the current Gemmini HP1 controller or a new cycle model. Legacy vector
> operations and HP1 fragment saturation are distinct; see the
> [current numerical contract](../NUMERICAL_CONTRACT.md) and
> [retained BSV contract notes](CONTRACT_NOTES.md). Board experiment results and
> deployment workflows are not part of the supported repository responsibilities.
...
Column별 synchronous BRAM을 `Accumulator` 내부 backend로 사용한다. 정수 profile은 DIM별 rows를 조정해64 KiB를 유지한다. 요청 수락과 writeback 완료를 분리하며 read response는 acknowledgement까지 보존한다. `accumulate=True` 전에 대상 row를 preload 또는 replace로 정의해야 한다. 자세한 BSV 계약은 `ARCHITECTURE.md` 참조. 현재 Gemmini 합성 경로는 `../SYNTHESIS.md`에 별도로 정의한다.
...
Global logical cycle, per-work interval, detailed counter, event timestamp의 정확한 source와 edge convention은 [RTL 사이클 측정 구조](RTL_CYCLE_ACCOUNTING.md)를 따른다.
...
- [Architecture](ARCHITECTURE.md): 현재 RTL 및 simulator architecture
- [코드 분석 가이드](CODE_ANALYSIS_GUIDE.md): 코드 분석 순서와 작성 규칙
- [검증 가이드](VERIFICATION.md): testbench 범위와 검증 상태
...
코드 구조의 분석·문서화 순서는 [코드 분석 가이드](CODE_ANALYSIS_GUIDE.md)를 따른다.
