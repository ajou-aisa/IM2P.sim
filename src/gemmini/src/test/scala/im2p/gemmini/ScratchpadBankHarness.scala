...
final class HarnessBankLoad(profile: ResolvedProfile, bankRows: Int) extends Bundle {
  val weights = Bool()
  val slot = Bool()
...
final class HarnessBankRead(bankRows: Int) extends Bundle {
  val slot = Bool()
  val row = UInt(math.max(1, log2Ceil(bankRows)).W)
}

// Test-only wiring around pinned ScratchpadBank; no GEMM controller or elaboration CLI.
final class ScratchpadBankHarness(profile: ResolvedProfile, bankRows: Int) extends Module {
  ...
  private val readType = new HarnessBankRead(bankRows)

  val io = IO(new Bundle {
    val load = Flipped(Decoupled(new HarnessBankLoad(profile, bankRows)))
  ...
  private def connectRead(
    request: DecoupledIO[HarnessBankRead],
