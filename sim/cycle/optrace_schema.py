...
from sim.cycle.optrace_parents import ParentError, ParentValidator
...
RUN = COMMON | {'schema','version','model','profile','activation_bits','weight_bits','dim',
                ...
                'source_commits','source_worktree_sha256','hardware_contract_sha256','runtime_manifest_sha256'}
...
WORK_NUMBERS = {'phase_id','activation_bits','weight_bits','dim','m','n','k','tile_i_count',
                ...
                'column_begin','group_index','logical_work_id','parent_invocation_id'}
...
def _parent_record(parents: ParentValidator, record: dict[str, Any]) -> None:
    try:
        parents.consume(record)
    except ParentError as error:
...
def validate_records(records: list[dict[str, Any]]) -> Trace:
    ...
    require(run.get('version') != 1,
            'legacy trace v1 lacks hardware binding and parent declarations; recollect with a current trace v2 producer')
    fields(run,RUN)
    ...
    for key in ('hardware_contract_sha256','runtime_manifest_sha256'):
        require(isinstance(run[key],str) and re.fullmatch('[0-9a-f]{64}',run[key]) is not None,
                'missing/invalid '+key)
    ...
    for sequence,r in enumerate(records):
        ...
        elif kind in ('parent_begin','parent_end'):
            pass
        elif kind=='run_end':
