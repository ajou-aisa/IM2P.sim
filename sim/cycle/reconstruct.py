...
def reconstruct(inputs: Inputs, outputs: ReplayOutputs) -> Record:
    ...
    full, potal = read_manifest(inputs.full_cpu.graph), read_manifest(inputs.potal.graph)
    compare_manifests(full, potal)
    proof_full = verify_provenance(inputs.full_cpu, 'FULL_CPU')
    proof_potal = verify_provenance(inputs.potal, 'POTAL_COLLECTION', inputs.npu.trace)
    ...
    before = {path: path.stat() for path in sources}
    with tempfile.TemporaryDirectory(prefix='reconstruct-', dir=outputs.results.parent) as directory, \
        ...
        with closing(sqlite3.connect(root/'index.sqlite')) as connection, ExitStack() as streams:
            ...
            cpu.load(inputs.full_cpu.log, full)
            cpu.load(inputs.potal.log, potal)
            ...
            for key, node in potal.nodes.items():
                ...
                if node_class == 'ORDINARY_CPU':
                    ...
                    emit(stream, {'kind': 'SERVICE', 'node_class': node_class, 'duration_source': 'FULL_CPU', 'resource_kind': 'CPU',
                                  'duration': {'worker_intervals': [duration_sample(sample) for sample in vector], 'aggregation': 'NOT_MODELED'},
            ...
            npu = NpuJoin(inputs.npu, potal, cpu).write(stream)
            ...
            summary: Record = {'schema': 'im2p-reconstruction-summary', 'version': 1, 'status': 'PASS',
                ...
                'full_cpu_provenance_sha256': sha256(inputs.full_cpu.provenance),
                'potal_provenance_sha256': sha256(inputs.potal.provenance), **npu,
                'source_artifacts': {'full_cpu_cycle_log': object_value(proof_full['artifacts'])['cycle_log'],
                    ...
                    'npu_results': {'sha256': sha256(inputs.npu.results)}},
        ...
        for path, original in before.items():
            current = path.stat()
            require(all(getattr(original, name) == getattr(current, name)
                        for name in ('st_dev', 'st_ino', 'st_size', 'st_mtime_ns', 'st_ctime_ns')), 'input changed during reconstruction')
