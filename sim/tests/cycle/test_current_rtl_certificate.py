...
class CurrentCertificateTest(unittest.TestCase):
    ...
    def test_capture_observer_keeps_real_fixture_main_and_numerical_checks(self) -> None:
        ...
        self.assertIn('run_rmd_ws_rtl_fixture(capability, &state, execute_scu, execute_runs);', capture)
        self.assertIn('run_bound_rmd_ws_rtl_fixture(capability, &state, execute, execute_scu,', capture)
