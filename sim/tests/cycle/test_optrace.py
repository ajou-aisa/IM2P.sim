...
from sim.cycle.optrace_parents import DESCRIPTOR_FIELDS


def sync_parent(rows: list[dict]) -> None:
    work = rows[3]
    rows[2].update({key:work[key] for key in DESCRIPTOR_FIELDS})
    rows[2]['m'] = work['geometry_m']
...
def records(bits: int = 8, dim: int = 16) -> list[dict]:
    ...
    run = dict(kind='run',sequence=0,run_id='test',schema='im2p-production-optrace',version=2,
               ...
               hardware_contract_sha256='a'*64,runtime_manifest_sha256='b'*64)
    phase = dict(kind='phase',sequence=1,run_id='test',phase_id=0,phase_kind='prefill',
                 decode_index=None,input_tokens=256)
    work = dict(kind='npu_work',sequence=3,run_id='test',phase_id=0,parent_invocation_id=0,layer='blk.0.ffn_down',
                ...
                source_row_begin=0,source_row_count=0,column_begin=0,group_index=0,logical_work_id=3,
                rmd_raw=False,host_integer_block_multiply=False)
    parent = dict(kind='parent_begin',sequence=2,run_id='test',phase_id=0,parent_invocation_id=0)
    parent_end = dict(kind='parent_end',sequence=4,run_id='test',phase_id=0,parent_invocation_id=0,status='success')
    end = dict(kind='run_end',sequence=5,run_id='test',status='success',reason='',work_count=1,
               independent_counts=[dict(phase_id=0,layer=work['layer'],provenance='dense_main',count=1)])
    rows = [run,phase,parent,work,parent_end,end]
    sync_parent(rows)
    return rows
...
class TraceTests(unittest.TestCase):
    ...
    def test_required_geometry_and_types(self):
        for field in ('tile_i_count','tile_j_count','tile_k_count','activation_stride_bytes','production_geometry_version'):
            r=records(); del r[3][field]
            with self.assertRaises(optrace.TraceError): self.parse(r)
        for value in (0,-1,True,'1',2**65):
            r=records(); r[3]['tile_k_count']=value
            with self.assertRaises(optrace.TraceError): self.parse(r)

    def test_schema_and_unknown_fields(self):
        for field,value in [('schema','other'),('version',1),('version',3),('version',True)]:
            r=records(); r[0][field]=value
            with self.assertRaises(optrace.TraceError): self.parse(r)
        r=records();r[3]['weights']=[1,2]
        with self.assertRaises(optrace.TraceError): self.parse(r)

    def test_sequence_and_phase_integrity(self):
        r=records();r[3]['sequence']=1
        with self.assertRaises(optrace.TraceError):self.parse(r)
        r=records();r[3]['phase_id']=1
        with self.assertRaises(optrace.TraceError):self.parse(r)
        r=records();r[1]['phase_kind']='decode';r[1]['decode_index']=0
        with self.assertRaises(optrace.TraceError):self.parse(r)

    def test_residual_contract(self):
        r=records(); w=r[3];w.update(provenance='residual',scope='residual_compact',k=32,original_block_id=96)
        r[-1]['independent_counts'][0]['provenance']='residual'
        sync_parent(r)
        self.parse(r)
        for field in ('rmd_raw','host_integer_block_multiply'):
            v=copy.deepcopy(r);v[3][field]=True
            with self.assertRaises(optrace.TraceError):self.parse(v)
        r[3]['original_block_id']=None
        with self.assertRaises(optrace.TraceError):self.parse(r)
    ...
    def test_no_observed_answer_in_model_input(self):
        r=records();r[3].update(observed_rtl_start=9000,observed_rtl_done=9001,observed_rtl_elapsed=1)
        t=self.parse(r)
    ...
    def test_real_vocabulary_work_exceeds_single_gemm_default_budget(self) -> None:
        r = records()
        r[3].update(m=1, n=50257, k=768, tile_i_count=1, tile_j_count=9,
        ...
        result = optrace._replay_fixture(trace, Path(os.environ['IM2P_CYCLE_LIBRARY']))
    ...
    def test_actual_library_replay_deterministic(self):
        ...
        for bits in (4,8):
            for dim in (16,32,64):
                ...
                sources={k:r[0][k] for k in ('source_commits','source_worktree_sha256','profile')}
                a=optrace._replay_fixture(t,library)
                b=optrace._replay_fixture(t,library)
                self.assertEqual(a['status'], 'FIXTURE_ONLY')
                self.assertEqual(a.get('validation_scope'), 'UNCERTIFIED_SYNTHETIC_FIXTURE')
