...
public:
  ...
  RmdRtlFixture();
  ggml::gemmini::rmd::StripePacketHandle packet(
      std::size_t stripe_id, std::size_t row_begin, std::size_t row_count) const;
  std::vector<std::int64_t> expected_correction(
      std::size_t row_begin, std::size_t row_count) const;
  std::vector<float> expected_merge(std::size_t row_begin, std::size_t row_count) const;
  void set_exponent(std::size_t column, std::size_t block, std::int16_t exponent);
