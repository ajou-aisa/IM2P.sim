...
struct Invocation {
  ...
  static bool ready(void *opaque, const exsia::StripeReadyEvent &event) noexcept {
    ...
    try {
      require(event.stripe_id == self.events.size() &&
              ...
              event.row_begin == (self.events.empty() ? 0 : self.events.back().row_end) &&
              event.row_end > event.row_begin && event.row_end <= self.args.I &&
              event.activation_metadata.has_value(), "invalid HP1 stripe publication");
#if !defined(GGML_GEMMINI_ENABLE_RMD) || GGML_GEMMINI_ENABLE_RMD == 0
      require(!event.rmd_packet && !event.direct_residual, "HP1 host was built with RMD OFF");
#endif
      if (self.run) {
        require(self.args.A.raw_data() && self.runtime.A.bytes &&
                self.args.A.bits == GGML_GEMMINI_ACTIVATION_BITS &&
                self.args.A.rows >= event.row_end && self.args.A.cols == self.args.K &&
                self.args.A.row_stride_bytes >= self.args.K &&
                self.args.A.raw_size() >= self.args.K &&
                event.row_end - 1 <= (self.args.A.raw_size() - self.args.K) /
                                      self.args.A.row_stride_bytes,
                "invalid HP1 activation publication");
        for (size_t row = event.row_begin; row < event.row_end; ++row)
          std::memcpy(self.runtime.A.bytes->data() + row * self.runtime.A.row_stride_bytes,
              static_cast<const uint8_t *>(self.args.A.raw_data()) + row * self.args.A.row_stride_bytes,
              self.args.K);
        const auto status = frontend::submit_stripe(*self.run, event,
            {true, event.activation_metadata->theta});
  ...
  void merge(const exsia::StripeReadyEvent &event, float *destination,
    ...
    if (!event.rmd_packet) return;
    const auto &packet = *event.rmd_packet;
    require(packet.stripe_id == event.stripe_id && packet.row_begin == event.row_begin &&
    ...
    metadata.theta.assign(event.stripe_id + 1, std::numeric_limits<int16_t>::min());
    ...
    auto status = hp1::execute_rmd_packet(residual, stripe, packet, correction, &metrics);
    require(status == rmd::RmdStatus::success, rmd::rmd_status_message(status));
    status = rmd::merge_rmd_correction_to(stripe, destination, packet, correction);
    require(status == rmd::RmdStatus::success, rmd::rmd_status_message(status));
  ...
  static frontend::Status merge_stage(void *opaque, im2p_sim_t *simulator,
      const exsia::StripeReadyEvent &event, frontend::ResidualStageView stage,
...
struct SinkBinding {
  ggml_gemmini_args_t &args;
  explicit SinkBinding(ggml_gemmini_args_t &value, const exsia::StripeReadySink &sink)
      : args(value) {
    require(!args.exsia_stripe_ready_sink, "HP1 invocation already has a stripe producer sink");
...
im2p::gemmini_hp1::Capability ggml_gemmini_hp1_capability() { return capability(); }

bool ggml_gemmini_hp1_bind_executor(const ggml_gemmini_hp1_executor &executor) {
  std::lock_guard<std::mutex> lock(adapter_mutex);
  last_error.clear();
  if (!hp1::compatible(capability(), executor.capability) || !executor.prepare ||
      !executor.full || !executor.stream.begin || !executor.stream.publish ||
      !executor.stream.poll || !executor.stream.finish || !executor.raw) {
...
bool ggml_gemmini_fpga_execute(ggml_gemmini_args_t &args, bool pipeline,
...
#if defined(GGML_GEMMINI_ENABLE_RMD) && GGML_GEMMINI_ENABLE_RMD != 0
  require(args.residual_route == ggml::gemmini::residual::ResidualRoute::ws_packet,
  ...
  if (args.D) {
    ...
    require(std::all_of(bias, bias + args.J, [](int32_t value) { return value == 0; }),
  ...
  Invocation invocation{args, runtime, {executor.capability, executor.context, executor.raw},
                        nullptr, {}, {}, {}};
  ...
  if (pipeline) {
    ...
    quantize();
    require(invocation.producer_failure.empty(), invocation.producer_failure.c_str());
    runtime = args;
    runtime.f_out = staged.data();
    runtime.D = nullptr;
    runtime.repeating_bias = false;
  ...
  auto launched = frontend::execute(&runtime,
      pipeline ? frontend::Mode::stripe_pipeline : frontend::Mode::full, options);
  ...
  require(!invocation.events.empty() && invocation.events.back().row_end == args.I,
          "incomplete HP1 quantized stripe coverage");
  if (pipeline) {
    const auto status = frontend::authorize_output_commit(*launched.run, true);
    require(status.ok(), status.message);
  } else {
  ...
  if (result_observer) result_observer(runtime, result_context);
