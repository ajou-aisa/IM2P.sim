...
void put_operand(std::vector<std::uint8_t> &bytes, std::size_t index,
                 std::int8_t value) {
...
struct Adapter {
  ...
  Adapter(Dut &model, VerilatedContext &simulation)
      : dut(model), context(simulation) {}
  ...
  std::vector<std::uint8_t> read_row(std::uint64_t address) {
    ...
    for (std::size_t slot = 0; slot < 2 && !source; ++slot) {
      for (const auto candidate :
           std::array{std::pair{slot_address(a_base, slot), &a[slot]},
    ...
    return {source->begin() + (address - base),
            source->begin() + (address - base) + width};
  }

  void write_row() {
    const auto address =
        static_cast<std::uint64_t>(dut.io_writeRequest_bits_address);
    ...
    for (std::size_t candidate = 0; candidate < 2; ++candidate) {
      ...
      if (address >= base && address < base + c[candidate].size())
        slot = candidate;
    }
    check(slot < 2, "RTL write outside padded INT32 C backing object");
    const auto offset =
        static_cast<std::size_t>(address - slot_address(c_base, slot));
    ...
    for (std::size_t i = 0; i < acc_bytes; ++i) {
      if ((byte_at(dut.io_writeRequest_bits_mask, i / 8) & (1U << (i % 8))) ==
          0)
        continue;
      const auto at = offset + i;
      check(at / c_stride[slot] < rows[slot] &&
                at % c_stride[slot] < columns[slot] * 4,
  ...
  void step() {
    dut.clock = 0;
    dut.io_readRequest_ready =
        read_ready_period == 0 || cycle % read_ready_period != 0;
    ...
    if (dut.io_error) {
      std::cerr << "WS protocol error phase=" << phase << " cycle=" << cycle
                ...
                << " stores=" << stores << " contexts=" << contexts
                << " raw=" << raw_rows << " commits=" << commits
                << " pending_reads=" << reads.size()
                << " pending_writes=" << writes.size()
                << " scale_release=" << unsigned(dut.io_scaleRelease_valid)
                << '\n';
      throw std::runtime_error("integrated WS RTL protocol error");
    }
    if (!writes.empty())
      check(!dut.io_logicalDone_valid,
            "logicalDone preceded backing write completion");
    ...
    if (dut.io_readBeat_valid && dut.io_readBeat_ready) {
      ...
      latest_read_sequence =
          std::max(latest_read_sequence, reads[selected].sequence);
    ...
    read_backpressure_cycles +=
        dut.io_readRequest_valid && !dut.io_readRequest_ready;
    if (stalled_read_valid) {
      check(dut.io_readRequest_valid &&
                static_cast<std::uint64_t>(dut.io_readRequest_bits_address) ==
                    stalled_read_address &&
                static_cast<std::uint32_t>(dut.io_readRequest_bits_beats) ==
                    stalled_read_beats &&
                static_cast<std::uint32_t>(dut.io_readRequest_bits_id) ==
                    stalled_read_id,
    ...
    if (dut.io_readRequest_valid && dut.io_readRequest_ready) {
      ...
      check(std::none_of(reads.begin(), reads.end(),
                         [id](const Read &r) { return r.id == id; }),
      ...
      reads.push_back({id, address,
                       cycle + read_latency +
                           (id % 2 == 0 ? reorder_latency : 0) +
                           (scale ? scale_latency : 0),
                       ++read_sequence, scale});
    }
    ...
    if (dut.io_events_loadDmaAccepted) {
      ++load_dma;
      accepted_load_local_addresses.push_back(
          dut.io_events_loadLocalAddressRaw);
    }
    store_dma += dut.io_events_storeDmaAccepted;
    if (dut.io_events_scratchpadReadAcceptedBankMask)
      scratchpad_read_bank_masks.push_back(
          dut.io_events_scratchpadReadAcceptedBankMask);
    if (dut.io_outputCompleted_valid) {
      const auto id = static_cast<std::size_t>(dut.io_outputCompleted_bits);
      check(id < output_ids.size() && !output_ids[id],
            "duplicate or unknown output completion");
      output_ids[id] = true;
      ++completions;
    }
  ...
  template <typename Ready> void accept(Ready ready, const char *message) {
    phase = message;
  ...
  void pack_scales(const FrontendRtlWork &work, std::size_t j0, std::size_t js,
    ...
    s[active_slot].assign((last_block - first_block + 1) * max_j * acc_bytes,
                          0);
    for (auto block = first_block; block <= last_block; ++block) {
      for (std::size_t j = 0; j < max_j; ++j) {
        for (std::size_t lane = 0; lane < dim && j * dim + lane < js; ++lane) {
          const auto carrier =
              work.carriers.at(block * work.columns + j0 + j * dim + lane);
  ...
  void release_scales(std::size_t js, std::size_t kb, std::size_t ke,
    ...
    for (std::size_t address = first_address; address < first_address + rows;
         ++address) {
      for (std::size_t lane = 0; lane < dim; ++lane) {
        ...
        accept([&] { return dut.io_scaleRelease_ready; },
               "scale release stalled");
  ...
  void loop(const FrontendRtlWork &work, std::size_t i0, std::size_t j0,
    ...
    const auto scale_base =
        work.plan.mode == Mode::pipeline ? active_slot * 128 : 0;
    ...
    dut.io_work_bits_cAddress = final_fragment
                                    ? slot_address(c_base, active_slot) +
                                          i0 * c_stride[active_slot] + j0 * 4
                                    : 0;
    ...
    check(reads.empty() && writes.empty(),
          "loop finished before backing traffic drained");
...
int execute(void *opaque, const FrontendRtlWork &work,
  ...
  check(work.rows && work.columns && work.k && work.plan.tile_i &&
            work.plan.tile_j && work.plan.tile_k,
  ...
  const auto old_scale_reads = state.scale_reads,
             old_scale_responses = state.scale_responses;
  ...
  check(state.logical_done - old_done == 1,
        "logical work completed more or less than once");
  check(state.contexts - old_contexts == expected_contexts &&
            state.raw_rows - old_raw == expected_rows &&
            state.commits - old_commits == expected_rows &&
            state.completions - old_completions == expected_outputs,
        "accepted output contexts/raw rows/commits differ from WorkPlan "
        "coverage");
  check(state.loads > old_loads && state.executes > old_executes &&
            state.stores > old_stores,
        "missing accepted upstream load/execute/store events");
  check(state.reordered_reads > old_reordered,
  ...
  check(std::any_of(state.accepted_read_addresses.begin() + old_addresses,
                    state.accepted_read_addresses.end(),
  ...
  if (work.plan.mode == Mode::pipeline &&
      work.plan.kind == WorkKind::dense_hp1_final) {
    state.pipeline_activation_bases.push_back(
        slot_address(a_base, state.active_slot));
  }
  check(state.dut.io_measurementValid && state.dut.io_elapsedCycles &&
            state.dut.io_doneCycle - state.dut.io_startCycle ==
                state.dut.io_elapsedCycles,
  ...
  for (std::size_t i = 0; i < state.c[state.active_slot].size(); ++i)
    check(state.written[state.active_slot][i] ==
              (i / state.c_stride[state.active_slot] < work.rows &&
                       i % state.c_stride[state.active_slot] < work.columns * 4
                   ? 1
                   : 0),
      ...
      for (std::size_t byte = 0; byte < 4; ++byte)
        raw |=
            static_cast<std::uint32_t>(
                state.c[state.active_slot]
                       [i * state.c_stride[state.active_slot] + j * 4 + byte])
            << (byte * 8);
      std::int32_t value;
  ...
  std::cout << "WS RTL " << (work.plan.mode == Mode::full ? "FULL" : "PIPELINE")
            ...
            << " tile=" << work.plan.tile_i << '/' << work.plan.tile_j << '/'
            << work.plan.tile_k << " load/ex/store=" << state.loads - old_loads
            << '/' << state.executes - old_executes << '/'
            << state.stores - old_stores << " contexts=" << expected_contexts
            << " committed_rows=" << expected_rows
...
int execute_raw(void *opaque, const RmdRawWork &raw,
  ...
  // Raw identity comes from immutable RTL work context, even with a hostile
  // scale.
...
int execute_scu(void *opaque, const RmdScuWork &scaled,
                std::vector<std::int32_t> &output, std::uint64_t &cycles) {
  check(scaled.plan.kind == WorkKind::dense_hp1_final,
        "production residual did not use normal HP1 work");
  FrontendRtlWork work{};
  work.work_id = scaled.work_id;
  work.host_slot = scaled.host_slot;
  work.rows = scaled.plan.m;
  work.columns = scaled.plan.n;
  work.k = scaled.plan.k;
  work.plan = scaled.plan;
  work.activations = scaled.activations;
  work.weights = scaled.weights;
  work.carriers = scaled.carriers;
  FrontendRtlTiming timing{};
  const auto status = execute(opaque, work, output, timing);
  cycles = timing.done_cycle - timing.start_cycle;
  return status;
...
void run_dual_loop_overlap(Adapter &state) {
  ...
  const auto held_address =
      static_cast<std::uint64_t>(state.dut.io_work_bits_aAddress);
  const auto held_scale =
      static_cast<std::uint32_t>(state.dut.io_work_bits_scaleBase);
  ...
  state.accept(
      [&] {
        return std::find(state.completed_slots.begin() + old_completed,
                         state.completed_slots.end(),
                         0U) != state.completed_slots.end();
      },
  ...
  state.dut.io_work_valid = 1;
  state.dut.eval();
  check(state.dut.io_work_ready, "slot0 did not become reusable after ordered "
                                 "completion and scale release");
  state.dut.io_work_valid = 0;
  ...
  state.accept(
      [&] {
        return std::find(state.completed_slots.begin() + old_completed,
                         state.completed_slots.end(),
                         1U) != state.completed_slots.end();
      },
      "slot1 loop completion stalled");
  state.release_scales(1, 0, 1, 0, 42, 128);
  state.accept([&] { return !state.dut.io_busy; },
               "dual-loop top did not drain");
  ...
  check(state.load_dma - old_load_dma == 2 * (dim + 1) &&
            ...
            state.contexts - old_contexts == 2 &&
            state.logical_done - old_done == 2,
        "dual-loop accepted DMA/context/completion counts differ from two WS "
        "works");
  ...
  check(lower_half && upper_half, "dual-loop loads did not occupy both "
                                  "original LoopMatmul scratchpad halves");
    ...
    check(std::any_of(state.accepted_read_addresses.begin() + old_reads,
                      ...
                      [&](std::uint64_t address) {
                        return address >= slot_address(a_base, slot) &&
                               address < slot_address(a_base, slot) +
                                             state.a[slot].size();
                      }) &&
              std::any_of(state.accepted_read_addresses.begin() + old_reads,
                          ...
                          [&](std::uint64_t address) {
                            return address >= slot_address(b_base, slot) &&
                                   address < slot_address(b_base, slot) +
                                                 state.b[slot].size();
                          }),
          "dual-loop backing trace missed immutable slot A/B objects");
  }
  std::cout
      << "WS_DUAL_LOOP slots=0,1 overlap_loop_issue=1 load_execute_overlap="
      << state.overlap - old_overlap
      << " slot0_reuse_blocked=1 delayed_done_cycles="
...
int main(int argc, char **argv) {
  try {
    ...
    dut.reset = 0;
    state.clock();
    const Capability capability{"ws-rtl-test",
                                "ws-rtl-test",
    ...
    FrontendRtlWork smoke{
        ...
        std::vector<std::int8_t>(128, 1),
        std::vector<std::int8_t>(64 * 3, 1),
        {30, 1, 0x80000000U, 1, 0, 2}};
    for (std::size_t k = 32; k < 64; ++k) {
      smoke.weights[k * 3 + 1] = 2;
      smoke.weights[k * 3 + 2] = -1;
    }
    const std::vector<std::int32_t> smoke_expected = {INT32_MAX, 128, -128,
                                                      INT32_MAX, 128, -128};
    ...
    if (smoke_output != smoke_expected) {
      ...
      for (const auto value : smoke_output)
        std::cerr << value << ',';
      std::cerr << '\n';
    }
    check(smoke_output == smoke_expected,
          "integrated K64 smoke output mismatch");
    ...
    const auto minimum_operand =
        static_cast<std::int8_t>(-(1 << (IM2P_OPERAND_BITS - 1)));
    ...
    check(execute_raw(&state, raw_boundary, raw_boundary_output,
                      raw_boundary_cycles) == IM2P_OK &&
              raw_boundary_output ==
                  std::vector<std::int32_t>{32 * minimum_operand *
                                            minimum_operand},
          "RMD K32 extremal dot narrowed or saturated");
    std::fill(raw_boundary.activations.begin(), raw_boundary.activations.end(),
              0);
    check(execute_raw(&state, raw_boundary, raw_boundary_output,
                      raw_boundary_cycles) == IM2P_OK &&
