...
int execute_rmd_raw_descriptor(void *opaque, const im2p_matmul_desc_t *descriptor,
                               im2p_work_stats_extended_t *stats) {
  if (!opaque || !descriptor) return IM2P_INVALID_LAYOUT;
  auto &executor = *static_cast<RmdExecutorContext *>(opaque);
  const auto &capability = executor.capability;
  const auto &d = *descriptor;
  if (!executor.execute || !capability.rmd || !capability.ws ||
      ...
      capability.packing != (GGML_GEMMINI_WEIGHT_BITS == 4
          ? Packing::signed_int4_low_nibble_first : Packing::signed_int8) ||
      d.abi_version != IM2P_ABI_VERSION || d.dim != capability.dim ||
      d.activation_bits != capability.activation_bits ||
      d.weight_bits != capability.weight_bits || d.activation_bits != d.weight_bits ||
      ...
      d.block_size != 1 ||
      !d.activations || !d.provider.read_weight_i8 || !d.provider.write_output ||
      d.scales || d.provider.read_scale || d.m == 0 || d.n == 0 || d.k == 0 || d.k > 32 ||
      ...
      d.m > std::numeric_limits<std::size_t>::max() / d.activation_row_stride_bytes ||
      d.n > std::numeric_limits<std::size_t>::max() / d.k) return IM2P_INVALID_LAYOUT;
  ...
  try {
    ...
    RmdRawWork work;
    work.plan = {d.m, d.n, d.k, geometry.tile_I, geometry.tile_J, geometry.tile_K,
                 d.m, Mode::full, WorkKind::rmd_raw};
    ...
    if (!std::all_of(work.activations.begin(), work.activations.end(), valid_code) ||
    ...
    const int status = executor.execute(executor.context, work, raw, cycles);
    ...
    for (std::size_t i = 0; i < d.m; ++i) {
      ...
      if (d.provider.write_output(d.provider.context, 0, i, 0, d.n, row.data(),
                                  IM2P_OUTPUT_LEGACY_FINAL) != IM2P_OK) return IM2P_ERROR;
    }
    if (stats) {
      *stats = {};
      ...
      stats->base.completed_output_tiles = ((d.m + DIM - 1) / DIM) * ((d.n + DIM - 1) / DIM);
      stats->base.completed_fragments = (d.k + std::min<std::size_t>(DIM, 32) - 1) /
...
ggml::gemmini::rmd::RmdStatus execute_rmd_packet(
    RmdExecutorContext &context, const ggml_gemmini_args_t &args,
  ...
  if (!context.capability.rmd || !context.execute ||
      (args.weight_format != Format::q4_hp1 && args.weight_format != Format::q8_hp1))
    return rmd::RmdStatus::unsupported_route;
  const rmd::Im2pFullExecutor executor{&context, execute_rmd_raw_descriptor};
  return rmd::execute_rmd_stripe_im2p(nullptr, args, packet, correction, metrics, &executor);
}
