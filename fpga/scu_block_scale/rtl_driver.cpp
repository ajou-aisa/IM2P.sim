...
#include "Vscu_uart_shell.h"
#include "Vscu_uart_shell___024root.h"
#include "verilated.h"
#include <cerrno>
#include <csignal>
#include <cstring>
...
constexpr size_t maximum_payload = 4 * 336 * 48;
...
struct Driver {
    ...
    void record(const char *event) {
        ...
        events << "{\"event\":\"" << event << "\",\"tick\":" << ticks
               << ",\"run_id\":" << root.scu_uart_shell__DOT__run_id
               << ",\"generation\":" << root.scu_uart_shell__DOT__generation
    ...
    void tick() {
        ...
        if (top.reset_n) {
            const auto &root = *top.rootp;
            if (root.scu_uart_shell__DOT__do_start) { ++launches; record("start"); }
            if (root.scu_uart_shell__DOT__do_publish) { ++publications; record("publish"); }
            if (root.scu_uart_shell__DOT__do_stripe_ack) { ++stripe_acks; record("stripe_ack"); }
        }
