...
struct State {size_t m=1,writes=0;};
...
int scale(void*,size_t b,size_t c,size_t n,uint32_t*out){ck(b<2&&c+n<=17,"scale extent");for(size_t i=0;i<n;++i)out[i]=b*101+c+i;return 0;}
int output(void *p,size_t b,size_t r,size_t c,size_t n,const int64_t*v,uint32_t domain){auto&s=*static_cast<State*>(p);++s.writes;ck(r<s.m&&b<2&&c+n<=17&&domain==IM2P_OUTPUT_LEGACY_BLOCK,"callback outside output");for(size_t i=0;i<n;++i)ck(v[i]==value(b,r,c+i),"MOCK value decode");return 0;}
int main(int argc,char**argv){try{
 ck(argc==3,"args");std::string mode=argv[2];auto api=im2p::fpga::window_uart_api();void*d=nullptr;
 ...
 State s;std::array<int8_t,320>a{};for(size_t i=0;i<a.size();++i)a[i]=int(i%101)-50;
 ...
 if(mode!="success"){
  ...
  if(mode=="extra_output")ck(s.writes==4,"extra output reached callback");
  else if(mode=="duplicate_tag"||mode=="skip_tag")ck(s.writes==1,"bad tag reached callback");
  else if(mode=="output_padding")ck(s.writes==2,"bad padding reached callback");
  else ck(s.writes==0,"fault leaked callback");
  std::cout<<"MOCK_PASS "<<mode<<" raw_callbacks="<<s.writes<<" rejected="<<api->error(d)<<"\n";api->destroy(d);return 0;
 }
 ck(status==0&&s.writes==4&&api->release(d)==0,"FULL/release");
 s.writes=0;ck(api->full(d,&f,&stats)==0&&s.writes==4&&api->release(d)==0,"repeated FULL");
...
#undef COPY
 ...
 ck(api->finish(d,&stats)==0&&s.writes==20&&stats.base.completed_stripes==5&&stats.base.stripe_rows_published==5,"live coverage");
 ck(api->release(d)==0,"live release");api->destroy(d);std::cout<<"MOCK_PASS success FULL=2 live_stripes=5\n";
...
def run(exe,mode):
    ...
    process=subprocess.Popen([str(exe),os.ttyname(slave),mode],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True)
    ...
    try:
        while process.poll() is None:
            ...
            if op==wire.CAP:
                ...
                assert args[3]==6|(5<<5)|(6<<10)|(6<<13)|((0 if mode=='bypass_scale' else 3)<<16)
                ...
                if op==wire.PUBLISH:
                    ...
                    for index in range(words):
                        ...
                        if kind==2:
                            expected+=struct.pack('<4I',*[row*101+column+i if row<2 and column+i<17 else 0 for i in range(4)])
                        else:
                            values=[]
                    ...
                    if stage=='refill':
                        ...
                        if mode=='output_padding':struct.pack_into('<i',records,2*wire.RECORD_BYTES+28,1)
                    ...
                    assert args==[state['batch'],4,0,0,0]
                    state['outputs']+=4;state['batch']+=1;state['stage']='stripe' if state['live'] else 'done'
                    reply=response(request)
        ...
        assert process.returncode==0 and f'MOCK_PASS {mode}' in output,output
        assert not select.select([master],[],[],0)[0],'unexpected packet after completion/failure'
        return dict(mode=mode,status='PASS',output=output.strip(),transactions=transactions)
    finally:
...
def main():
    ...
    paths=[Path(__file__),ROOT/'fpga/scu_block_scale/window_uart.cpp',ROOT/'fpga/scu_block_scale/window_uart.hpp',ROOT/'fpga/scu_block_scale/window_protocol.hpp',ROOT/'fpga/scu_block_scale/window_packets.py']
    ...
    try:
        ...
        for mode in args.mode or ('success','cap_version','cap_profile','version','operation','sequence','generation','run','profile','padding','flags','crc','output_address','output_tag','duplicate_tag','skip_tag','output_padding','coverage','extra_output','window_origin','window_alignment','window_generation','scale_origin','bypass_scale','batch_identity'):
