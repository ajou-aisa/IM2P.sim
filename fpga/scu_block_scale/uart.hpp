#pragma once
// IFR3 target derived explicitly from fpga/dense_pipeline/uart.hpp (IFR2).
...
struct FullCycleExpectation {
    ...
    size_t m, n, k;
    uint64_t cycles;
    std::string hardware_sha256, production_rtl_sha256;
};
...
// One persistent, serialized IFR3 connection. Construction performs OPEN/CAP.
...
public:
    ...
    bool is_rtl() const noexcept { return rtl_library_ != nullptr; }
    void set_block_observer(im2p_scu_rtl_output_observer_v1 callback, void *context);
    std::vector<int32_t> last_raw; // final row/column; valid until the next full()/begin().
...
private:
    void *rtl_library_ = nullptr, *rtl_instance_ = nullptr;
    const im2p_scu_rtl_api_v1 *rtl_api_ = nullptr;
    void observe_rtl();
    void rtl_status(int status);
    void start_rtl_run(size_t m, size_t n, size_t k, bool streaming);
    void finish_rtl_run(const im2p_work_stats_extended_t &stats);
    int fd_ = -1;
