// IFR3 target copied explicitly from fpga/dense_pipeline/host_dispatch.cpp; legacy source is preserved.
...
struct Observation {
    ...
    size_t reference_matmul_calls = 0, reference_dot_calls = 0, reference_scu_gemm_calls = 0;
    ...
    static void complete(const ggml_gemmini_args_t &args, const std::vector<int32_t> &raw, void *opaque) {
        ...
        require(raw.size() == args.I * args.J, "SCU final raw count");
        ...
        if (args.I > 32) {
            ...
            require(geometry.ok() && meta.theta.size() == geometry.geometry.stripe_count && !meta.theta.empty(),
                    "completed invocation stripe geometry mismatch");
            require(std::none_of(meta.theta.begin(), meta.theta.end(), [](int16_t theta) {
                        return theta == std::numeric_limits<int16_t>::min(); }) &&
                    std::adjacent_find(meta.theta.begin(), meta.theta.end()) == meta.theta.end(),
                    "long invocation requires distinct adjacent valid stripe theta");
        }
        auto compare_output = [&] {
            for (size_t row = 0; row < args.I; ++row)
                for (size_t col = 0; col < args.J; ++col) {
                    const size_t index = row * args.stride_f_out + col * args.col_stride_f_out;
                    require(std::bit_cast<uint32_t>(args.f_out[index]) ==
                            std::bit_cast<uint32_t>(self.expected[index]),
                            "G2 staged f_out float32 bit-exact mismatch");
                }
        };
        ...
        self.expected_raw.assign(raw.size(), 0);
        const auto geometry = args.activation_geometry();
        require(geometry.ok(), "G1 activation geometry");
        size_t scu_clamps = 0, accumulator_clamps = 0;
        auto clamp32 = [](int64_t value, size_t &count) {
            if (value > INT32_MAX) { ++count; return INT32_MAX; }
            if (value < INT32_MIN) { ++count; return INT32_MIN; }
            return int32_t(value);
        };
        // Test-only G1: scalar integer arithmetic after actual FPGA completion.
        // No host MatMul/CPU backend produces the selected invocation's output.
        // A8/D16 partial * H1 uint17 fits int64 before either explicit clamp.
        for (size_t row = 0; row < args.I; ++row) {
            ...
            for (size_t col = 0; col < args.J; ++col) {
                ...
                for (size_t first = 0; first < args.K; first += 16) {
                    ...
                    const int32_t scaled = clamp32(partial * int64_t(beta), scu_clamps);
                    accumulator = first == 0 ? scaled : clamp32(int64_t(accumulator) + scaled, accumulator_clamps);
                }
                const size_t index = row * args.J + col;
                self.expected_raw[index] = accumulator;
                require(raw[index] == accumulator, "G1 final signed32 exact mismatch");
                // G2 order is contractual: integer, shared S, activation scale.
                self.expected[row * row_stride + col * args.col_stride_f_out] =
                    float(double(accumulator) * double(shared) * double(activation_scale));
            }
        }
        ++self.reference_scu_gemm_calls;
        std::cout << "SCU_INDEPENDENT_REFERENCE numerical_revision=signed-scu-sat-v2"
                  << " raw_domain=signed32 output_domain=2 reference_scope=after_actual_execution"
                  << " scu_clamps=" << scu_clamps << " accumulator_clamps=" << accumulator_clamps
                  << " raw=" << raw.size() << " f_out=" << args.I * args.J << '\n';
...
struct Graph {
    ...
    void prepare(size_t seed) {
        ...
        if (m > 32) {
            ...
            const auto geometry = args.activation_geometry();
            require(geometry.ok() && geometry.geometry.stripe_count > 0,
                    "long input requires valid automatic stripe geometry");
...
int main(int argc, char **argv) {
    try {
        require(argc >= 2, "usage: host_dispatch reject | reject-hp1 | reject-activation | reject-metadata | failure M N K [MODE] | run M N K SEED REPS MODE | benchmark M N K SEED [REPS=5] FULL|STRIPE_PIPELINE");
        const std::string command = argv[1];
        Observation observation;
        ggml_gemmini_fpga_set_observer(Observation::complete, &observation);
        if (command == "reject-hp1") {
            require(argc == 2, "HP1 rejection arguments");
            require(unsetenv("IM2P_FPGA_DEVICE") == 0, "clear rejection-test device");
            std::vector<block_q8_hp1> weights(16);
            std::vector<float> output(16 * 19, 17.0f);
            ggml_gemmini_args_t args{};
            args.I = args.J = 16; args.K = 32;
            args.weight_format = ggml_gemmini_args_t::im2p_weight_format_t::q8_hp1;
            args.q8_hp1_blocks = weights.data(); args.q8_hp1_block_count = weights.size();
            args.q8_hp1_blocks_per_row = 1; args.native_weight_bytes = weights.size() * sizeof(block_q8_hp1);
            args.f_out = output.data(); args.stride_f_out = 19; args.col_stride_f_out = 1;
            require(args.A.allocate(16, 32, 8), "HP1 rejection A allocation");
            size_t quantization_calls = 0;
            for (bool pipeline : {false, true}) {
                require(!ggml_gemmini_fpga_execute(args, pipeline, [&] { ++quantization_calls; }, "HP1-unsupported"),
                        "IFR3 accepted unsupported HP1");
                require(ggml_gemmini_fpga_last_error() == "FPGA requires native Q8_H1", "HP1 rejected too late");
            }
            require(quantization_calls == 0 && observation.calls == 0 && simulator_creates == 0 &&
                    simulator_executes == 0 && simulator_streams == 0 &&
                    std::all_of(output.begin(), output.end(), [](float x) { return x == 17.0f; }),
                    "unsupported HP1 executed or changed output");
            std::cout << "IFR3_HP1_REJECT PASS cases=2 quantization_calls=0 simulator_calls=0 device_selected=0\n";
        } else if (command == "reject-activation" || command == "reject-metadata" || command == "reject-bias") {
            ...
            std::cout << "FPGA_UART_GGML_" << (command == "failure" ? "FAILURE" : "NUMERICAL")
                      ...
                      << " simulator_creates=0 simulator_executes=0 simulator_stream_begin=0"
                      << " protocol=3 semantic_capability=0294 numerical_revision=signed-scu-sat-v2 output_domain=2"
                      << " validation=" << (benchmark ? "reference_warmup_cached_check_measured" : "independent_scu_g1_g2_after_fpga")
                      << " reference_dot_calls=" << observation.reference_dot_calls
                      << " reference_matmul_calls=" << observation.reference_matmul_calls
                      << " reference_scu_gemm_calls=" << observation.reference_scu_gemm_calls << '\n';
            if (benchmark) {
