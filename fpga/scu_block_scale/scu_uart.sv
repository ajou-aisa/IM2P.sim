// IFR3: one logical GEMM, existing core scheduling, bounded full A/C backing.
module scu_uart_shell #(parameter integer UART_DIV = 25) (
    ...
    reg [3:0] state;
    reg [255:0] header;
    reg [15:0] index, input_bytes, a_bytes, scale_start;
    reg scale_pending;
    reg [7:0] scale_element;
    ...
    wire load_a_ready, load_w_ready, load_s_ready, start_ready, done, error, request_ready, output_valid;
    ...
    wire base_ok = header[31:0] == 32'h33524649 && header[39:32] == 3 && header[63:48] == 16'h0810;
    ...
    wire full_ok = op == 1 && shape_ok && t == 0 && length == x*128 + z*64 + (z >> 5)*256;
    wire begin_ok = op == 4 && shape_ok && t > 0 && t <= 336 && t[3:0] == 0 && length == z*64 + (z >> 5)*256;
    ...
    wire do_load_s = state == LOAD && scale_pending && load_s_ready;
    wire do_start = state == START && start_ready && !poison && !error;
    ...
    wire [10:0] output_address = ((raw_row_begin + output_row) << 2) + output_tile;
    ...
    wire [1151:0] reply = {overlap, host_wait, first_a_rows, first_a, completion_cycle, publish_cycle,
        ...
        16'h0294, 9'd0, reduction, 10'd0, columns, 7'd0, rows, reply_bytes,
        generation, run_id, 16'h0810, status, 8'd3, 32'h3352464f};
    ...
    mkScuPipeline core (.CLK(clk), .RST_N(reset_n && !core_reset),
        ...
        .loadScale_element(scale_element), .loadScale_value(scale_value),
        .EN_loadScale(do_load_s), .RDY_loadScale(load_s_ready),
    ...
    always @(posedge clk) begin
        if (!reset_n) begin
            ...
            input_bytes <= 0; a_bytes <= 0; scale_start <= 0;
            scale_pending <= 0; scale_element <= 0; scale_value <= 0; output_words <= 0; words_sent <= 0;
            output_tile <= 0; output_row <= 0; raw_row_begin <= 0; raw_rows <= 0;
            ...
            if (state == RX || state == LOAD) begin
                ...
                if (&frame_wait || rx_error) begin
                    ...
                    if (state == RX && index < 32) begin
                        ...
                        if ((op == 1 || op == 4) && index >= scale_start) begin
                            if (index[1:0] == 3) begin
                                if (scale_pending || {rx_data,assemble[127:104]} > 32'd65790) poison <= 1;
                                scale_value <= {rx_data,assemble[127:104]};
                                scale_element <= (index - scale_start) >> 2;
                                scale_pending <= 1;
                            end
                        end else if (index[3:0] == 15) begin
                ...
                if (state == RX && index == 32 && (op == 1 || op == 4 || op == 5)) begin
                    if (!request_header_ok || poison || ((op == 1 || op == 4) && !new_job_ok)) begin
                        ...
                        scale_start <= (op == 1 ? x*128 : 0) + z*64;
                        state <= LOAD; index <= 0;
                    end
                end
            end
            if (do_load_a || do_load_w) load_pending <= 0;
            if (do_load_s) scale_pending <= 0;
            if (state == CHECK) begin
                ...
                if (!request_header_ok) begin status <= 2; poison <= 1; end
                    ...
                    if (!new_job_ok || load_pending || scale_pending || error) begin status <= 5; poison <= 1; end
                    else begin
                    ...
                    if (stripe_valid) begin
                        if (core_stripe != retired_stripes || core_row + core_rows > rows || core_rows == 0) begin
                            ...
                            output_words <= core_rows * tiles;
                            reply_bytes <= (core_rows * tiles) << 6;
                            output_row <= 0; output_tile <= 0; words_sent <= 0;
                        end
                    end
            ...
            if (tx_valid && tx_ready) begin
                ...
                if (state == OUTPUT && send_index == 63) begin
                    ...
                    if (output_tile + 1 == tiles) begin
                        output_tile <= 0;
                        if (output_row + 1 == raw_rows) begin output_row <= 0; end
