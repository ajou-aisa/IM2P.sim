#!/usr/bin/env python3
"""IFR3 PTY tests derived from dense_pipeline/test_stream_uart.py; no RTL numerical evidence."""
...
bool bad_scale=false;
int scale(void*,size_t block,size_t col,size_t count,uint32_t*out){for(size_t lane=0;lane<count;++lane)out[lane]=bad_scale?65791:(block==0?col+lane:block==1?256+col+lane:65790);return 0;}
int out(void *p,size_t b,size_t r,size_t c,size_t count,const int64_t *v,uint32_t domain){if(domain!=2)return -1;auto&o=*static_cast<P*>(p);if(o.writes>=o.expected.size()||o.expected[o.writes++]!=Tuple(b,r,c,count))return -1;for(size_t l=0;l<count;++l)if(v[l]!=int64_t(r*100+c+l)-5000)return -1;return 0;}
int main(int argc,char**argv){try {
 std::string mode=argv[2];size_t m=mode=="large"?321:33,n=mode=="large"?48:19,k=mode=="large"?96:64,sr=mode=="large"?160:16;
 P p;for(size_t i=0;i<m;i+=16)for(size_t j=0;j<n;j+=16)for(size_t b=0;b<1;++b)for(size_t r=i;r<std::min(i+16,m);++r)p.expected.emplace_back(b,r,j,std::min(size_t(16),n-j));
 std::vector<uint8_t>a(m*k);for(size_t i=0;i<a.size();++i)a[i]=i%251;
 im2p_stripe_work_desc_t d{};d.abi_version=5;d.activation_bits=d.weight_bits=8;d.activation_storage_bytes=d.weight_storage_bytes=1;d.dim=16;d.m=m;d.n=n;d.k=k;d.tile_i_rows=d.tile_j_columns=16;d.block_size=32;d.vector_op=4;d.output_domain=2;d.scale_total_k=k;d.scale_row_stride=d.scale_valid_columns=d.weight_row_stride_bytes=d.output_row_stride=n;d.stripe_count=3;d.provider={&p,w,nullptr,scale,out};
 im2p::fpga::UART u(argv[1],3.0);im2p_work_stats_extended_t stats{};
 if(mode=="invalid"||mode=="hp1"||mode=="domain"||mode=="abi"||mode=="scale"){
 if(mode=="invalid")d.m=337;else if(mode=="hp1")d.vector_op=5;else if(mode=="domain")d.output_domain=1;else if(mode=="abi")d.abi_version=4;else bad_scale=true;ck(u.begin(&d,sr)<0,"invalid accepted");ck(u.metrics().transactions==0,"invalid transmitted");std::cout<<"IFR3_PTY PASS "<<mode<<" mock_framing_only=1\n";return 0;}
 ...
 auto stripe=[&](int id){im2p_activation_stripe_t s{};s.abi_version=5;s.activation_bits=s.weight_bits=8;s.activation_storage_bytes=s.weight_storage_bytes=1;s.dim=16;s.stripe_id=id;s.i_start=id*sr;s.rows=std::min(sr,m-s.i_start);s.activations=a.data()+s.i_start*k;s.activation_row_stride_bytes=k;s.context=0x12345678;return s;};
 ...
 ck(u.finish(&stats)==0&&p.writes==p.expected.size()&&u.last_raw.size()==m*n,"finish/raw callbacks");ck(stats.base.completed_stripes==3&&stats.base.stripes_published==3&&stats.base.stripe_rows_published==m,"stripe counts");ck(u.last_first_activation_cycle==101&&u.last_first_activation_published_rows==sr,"observations");
 ...
 im2p_matmul_desc_t f{};f.abi_version=5;f.activation_bits=f.weight_bits=8;f.activation_storage_bytes=f.weight_storage_bytes=1;f.dim=16;f.m=m;f.n=n;f.k=k;f.tile_i_rows=f.tile_j_columns=16;f.block_size=32;f.vector_op=4;f.output_domain=2;f.scale_total_k=k;f.scale_row_stride=f.scale_valid_columns=f.weight_row_stride_bytes=f.output_row_stride=n;f.activations=a.data();f.activation_row_stride_bytes=k;f.provider=d.provider;p.writes=0;
 ...
 std::cout<<"IFR3_PTY PASS "<<mode<<" raw="<<m*n<<" callbacks="<<p.writes<<" mock_framing_only=1\n";
 }catch(const std::exception&e){if(std::string(argv[2]).starts_with("cap_")){std::cout<<"IFR3_PTY PASS "<<argv[2]<<" rejected="<<e.what()<<" mock_framing_only=1\n";return 0;}std::cerr<<e.what()<<'\n';return 1;}}
'''
def rd(fd,n):
...
def recv(fd,op,run=1,gen=6):
    ...
    assert head[:8]==b'IFR3'+bytes([3,op,16,8]),head
...
def reply(op,shape,run=1,gen=6,payload=b'',counts=(0,)*7,idrow=None,first=0,firstrows=0):
    b=bytearray(144);struct.pack_into('<4sBBHQIIHHHH',b,0,b'OFR3',3,0,0x810,run,gen,len(payload),*shape,0x294)
...
def raw(m,n,k,first,rows):
    return b''.join(struct.pack('<i',r*100+c-5000 if c<n else 0) for r in range(first,first+rows) for c in range(((n+15)//16)*16))
def run(exe,mode):
    ...
    try:
        recv(master,0,0,0)
        capability=bytearray(reply(0,(336,48,96),0,5))
        if mode=='cap_v2':capability[:5]=b'OFR2'+bytes([2])
        if mode=='cap_semantic':struct.pack_into('<H',capability,30,0x214)
        if mode.startswith('cap_'):capability[-4:]=struct.pack('<I',zlib.crc32(capability[:-4]))
        send(master,capability)
        if mode not in ('invalid','hp1','domain','abi','scale','cap_v2','cap_semantic'):
            b=recv(master,4);assert struct.unpack_from('<4H',b,20)==(m,n,k,sr)
            expected=bytearray(k*64+(k//32)*256)
            for r in range(k):expected[r*64:r*64+n]=bytes((r*3+c)%127 for c in range(n))
            for block in range(k//32):
                for c in range(n):struct.pack_into('<I',expected,k*64+block*256+c*4,c if block==0 else 256+c if block==1 else 65790)
            ...
            if mode in ('success','large'):
                ...
                tiles=(n+15)//16;works=((m+15)//16)*tiles;writes=m*tiles;counts=(9999,works*(k//16),works,1,1,writes,writes)
                ...
                full=recv(master,1,2,7)
                full_expected=bytearray(m*128)+expected
                for r in range(m):full_expected[r*128:r*128+k]=bytes((r*k+c)%251 for c in range(k))
                assert full[32:-4]==full_expected
                send(master,reply(1,sh,2,7,payload=raw(m,n,k,0,m),counts=counts,first=101,firstrows=m))
        ...
        assert f'IFR3_PTY PASS {mode}' in out,out
        assert not select.select([master],[],[],0)[0], 'unexpected command after completion/failure'
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='im2p-ifr3-pty-') as temporary:
        ...
        command = ['c++', '-std=c++20', '-O2', '-I'+str(ROOT/'sim/include'),
                   '-I'+str(ROOT/'fpga/scu_block_scale'), str(source),
                   str(ROOT/'fpga/scu_block_scale/uart.cpp'), '-o', str(exe), '-pthread']
        (out/'command.json').write_text(json.dumps(command, indent=2)+'\n')
        identities = {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
                      for p in (Path(__file__).resolve(), ROOT/'fpga/scu_block_scale/uart.cpp',
                                ROOT/'fpga/scu_block_scale/uart.hpp', ROOT/'sim/include/im2p_sim.h')}
        ...
        with (out/'results.log').open('x') as log:
            for mode in ('success','large','invalid','crc','identity','profile','count','generation',
                         ...
                         'hp1','domain','abi','scale','cap_v2','cap_semantic'):
        ...
        (out/'status.json').write_text(json.dumps({'status':'PASS','tests':20,
